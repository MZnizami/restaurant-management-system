...
public class loginAndSignupController implements Initializable {
  ...
  //loginpage
    ...
    Connection con=null;
    PreparedStatement pst, pst2=null,pst3;
    ResultSet rs,rs2;
...
/*this button will get the text from the filed check them if the typed text is 
 ...
 try {     
             ...
             String waiterCheck= "SELECT * FROM `waiter-register` WHERE Name=? AND Password =?";
                pst3=(com.mysql.jdbc.PreparedStatement) con.prepareStatement(waiterCheck);
              pst3.setString(1, logName.getText());
              pst3.setString(2, logPassword.getText());
 ...
 ((Stage)(((Button)event.getSource()).getScene().getWindow())).close(); 
             ...
             root= FXMLLoader.load(getClass().getResource("waiterPage.fxml"));
              Stage s= new Stage();
              Scene scene = new Scene(root);
              s.setTitle("                             Waiter Main Page");
               s.setScene(scene);
                s.show();
 ((Stage)(((Button)event.getSource()).getScene().getWindow())).close(); 
...
///////////////////////////////////////Sign Up Page User
        @FXML
    private AnchorPane signupParnet;
 @FXML
    private ImageView signupImg;
     @FXML
    private AnchorPane siginupChild;
